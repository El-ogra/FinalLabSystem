...
هذه المهارة **الخطوة الثانية** في سلسلة تحليل وتنفيذ نظام مرجعي:

```
1) lis-pdf-reference-extractor   (تُستخدم أولاً — لاستخراج PRD وفجوات من PDF مرجعي)
        │
        ▼
2) lis-work-plan-with-validation-gates  ← أنت هنا (تُستخدم ثانياً)
        │
        ▼   (ينتج: Work Plan بشرائح مرتبة + Validation Gates)
...
- إذا كان المصدر **PDF مرجعي لنظام LIS**، **يجب** أن تكون `lis-pdf-reference-extractor` قد استُدعيت أولاً وأنتجت PRD في `FinalLabSystem/Docs/PRDs/<ModuleName>/`.
- إذا كان المصدر PRD قائم بالفعل (لا PDF)، تستدعى هذه المهارة مباشرة.
- **مخرج هذه المهارة (Work Plan)** يُغذّي مهارة `implement` لتنفيذ كل شريحة.

### تحويل صريح للمهارة التالية

عند نهاية جلسة Work Plan، أنهِ ردّك بالجملة التالية حرفياً:

> «Work Plan جاهز مع Validation Gates لكل شريحة. لبدء تنفيذ الشريحة الأولى، استدعِ الآن مهارة **`implement`**.»
